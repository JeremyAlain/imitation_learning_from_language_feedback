...
def main() -> None:
    ...
    def load_dataset(data_files: Dict[str, str]) -> Dict[str, List[Tuple[str, str]]]:
        raw_datasets: Dict[str, List[Tuple[str, str]]] = {}
            ...
            with jsonlines.open(input_path, "r") as reader:
                for row in reader:
                    ...
                    current_examples.append(
                        (A_input, B_input,)
                        if row["comparison_preference"] == "Summary A"
                        else (B_input, A_input,)
                    )
    ...
    class RewardModelCollator:
        ...
        def __call__(self, batch):  # type: ignore
            ...
            for input_pair in batch:
                all_batch_sequences.append(input_pair[0])
                all_batch_sequences.append(input_pair[1])
    ...
    eval_dataloader = DataLoader(  # type: ignore
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            with accelerator.accumulate(model):
                ...
                loss = calculate_reward_model_loss(outputs.logits)
        ...
        if not args.not_validate_after_epoch:
            accuracy = evaluate_model_on_validation_set(
                model, eval_dataloader, accelerator, epoch, steps, progress_bar, args,
            )
...
def evaluate_model_on_validation_set(  # type: ignore
    model, eval_dataloader, accelerator, epoch, completed_steps, progress_bar, args
):
    ...
    for step, batch in enumerate(eval_dataloader):
        ...
        cur_predictions = get_predictions(outputs.logits)
    ...
    # logger.info(f"epoch {epoch}: Validation Accuracy {validation_accuracy}")
    desc_str = "----------------------------"
...
def get_predictions(logits):  # type: ignore
    logits = logits.squeeze()
    B = logits.shape[0]
    summary_1_logits = logits[torch.arange(0, B, 2)]
    summary_2_logits = logits[torch.arange(1, B, 2)]
    return (summary_1_logits > summary_2_logits).long()


def calculate_reward_model_loss(logits):  # type: ignore
    logits = logits.squeeze()
    B = logits.shape[0]
    summary_1_logits = logits[torch.arange(0, B, 2)]
    summary_2_logits = logits[torch.arange(1, B, 2)]
    log_sigmoid = torch.nn.LogSigmoid()
    return -torch.mean(log_sigmoid(summary_1_logits - summary_2_logits))
