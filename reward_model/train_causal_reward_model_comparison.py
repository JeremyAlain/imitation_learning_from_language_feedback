    ...
    class RewardModelCollator:
        ...
        def __call__(self, batch):  # type: ignore
            ...
            for elem0 in batch:
                ...
                all_batch_labels.append(1 if "A" in elem0["completion"] else 0)
            ...
            return (input_elems, target_elems, torch.Tensor(all_batch_labels))
...
def evaluate_model_on_validation_set(  # type: ignore
    ...
    for step, batch in enumerate(eval_dataloader):
        input_ids, labels, completions = batch
        with torch.no_grad():
            outputs = model(**input_ids, use_cache=False)
        loss = calculate_reward_model_loss(
            outputs.logits, labels, args.prompt_loss_weight
        ...
        if accelerator.num_processes > 1:
            if step == len(eval_dataloader):
                ...
                completions = completions[: len(eval_dataloader.dataset) - samples_seen]
            else:
                samples_seen += cur_predictions.shape[0]
        predictions += cur_predictions.detach().tolist()
        references += completions.detach().tolist()
        total_steps += 1

    validation_accuracy = accuracy_score(references, predictions)
...
def get_predictions(input_ids, logits, tokenizer):  # type: ignore
    ...
    return (
        probs[torch.arange(B), sequence_lengths, first_token]
        > probs[torch.arange(B), sequence_lengths, second_token]
    ).long()
