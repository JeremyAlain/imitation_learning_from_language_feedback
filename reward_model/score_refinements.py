...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--num_samples", type=int, default=64, help="Number of summary samples.",
    )
        ...
        with jsonlines.open(input_file, "r") as reader:
            for row in reader:
                ...
                for i in range(args.num_samples):
                    if i > 31:
                        generated_summary = row[f"summary_{i}"]
                    else:
                        generated_summary = row[f"generated_summary_{i}"]
                    examples.append(
                        (
                            idx,
                            i,
...
def export_output(model_outputs, args):  # type: ignore
    ...
    with jsonlines.open(args.output_file, "w") as output_file:
        for elem in input_data:
            ...
            elem["selected_summary"] = int(np.argmax(annotations[:-1]))
            for i, annotation in enumerate(annotations):
                if i == args.num_samples:
                    elem["human_generated_summary_score"] = annotation
                else:
                    elem[f"generated_summary_{i}_score"] = annotation
